...
Kai implements a [Retrieval Augmented Generation (RAG)](https://www.promptingguide.ai/techniques/rag) approach that leverages data from Konveyor to help generate code suggestions to aid migrating legacy code bases to a different technology. Retrieval-Augmented Generation (RAG) is a pattern of providing extra information in a prompt to enhance a Large Language Model (LLM) to produce bettter or more relevant results on data it did not see previously in training. The intent of this RAG approach is to shape the code suggestions to be similar to how an organization has solved problems in the past, without additional fine-tuning of the Large Language Model (LLM).

Overview of Kai:

- Guided workflow uses discovered analysis issues to drive LLM interactions
- Generates code suggestions for discovered migration issues
  - Works with LLMs using structured migration data Konveyor has collected across the Organization.
- Avoids fine-tuning each model by using a RAG pattern.
  - Leverages knowledge of specific problems that need to be addressed based on Migration Rules.
  - Extracts patterns from previously solved problems to shape LLM results.
- Model agnostic, ability to bring your own model
